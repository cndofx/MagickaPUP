...
namespace MagickaPUP.MagickaClasses.Character
{
    ...
    public class CharacterTemplate : XnaObject
    {
        ...
        public CharacterTemplate()
        {
            ...
            this.EquipmentAttachments = new EquipmentAttachment[0]; // NOTE : We used to hard code this to 8, just like Magicka does. We can have 8 attachments per character at most. The thing is that the read side actually is allowed to input as many attachments as it wants, it's just that each entry has to identfy which attachment slot it affects. This means that we could have 10000 entries, but each would only be capable of pointing to one of the 8 valid slots, so they overwrite eachother actually... Note that in the future, we could optionally throw an exception and manually limit this to only 8 entries if it were to make sense, but on the JSON side only. That way, if an user writes a JSON file with more than 8 entries, we could get a warning. Altough, this could potentially cause problems. For example, since Magicka allows the XNB files to have as many entries as it pleases, if decompile an XNB file and then try to recompile the resulting JSON and that XNB happened to have more entries than allowed, we would trigger the error unnecessarily... so yeah, better to keep it this way, even tho it could potentially be exploited to freeze the game by extending load times in an absurd manner by adding tons of entries... in any case, people would eventually do that manually, so capping it on my tool is not really going to help anyone... Oh and ofc don't forget that people could always manually write an item index that is out of bounds, and that would crash, so yeah, maybe we should validate that with exceptions too within the equipment class!
        ...
        {
            return new ContentTypeReaderStorage("Magicka.ContentReaders.CharacterTemplateReader, Magicka, Version=1.0.0.0, Culture=neutral", 0);
        }
